...
function splitFormData(request, log=console.log) {
  ...
  if (!match) {
    log('I can\'t parse this Content-Type:', request.headers['Content-Type'])
    return
  }
  ...
  if (match[0].length != request.headers['Content-Type'].length) {
    ...
    log('Ignoring trailer after Content-Type:', trailer)
  }
